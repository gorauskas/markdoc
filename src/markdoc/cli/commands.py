...
init.parser.add_argument('destination', default=None,
                         help="Create wiki here (if omitted, defaults to "
                         "current directory)")
...
def vcs_ignore(config, args):
    ...
    if args.output == '-':
        ...
        log.info('Writing ignore file to %s' %
                 p.relpath(filename, start=wiki_root))
        fp = open(filename, 'w')
...
vcs_ignore.parser.add_argument('vcs', default='hg', nargs='?',
                               ...
                               help="Create ignore file for specified VCS "
                               "(default 'hg')")
vcs_ignore.parser.add_argument('-o', '--output', default=None,
                               metavar='FILENAME',
                               help="Write output to the specified filename, "
                               "relative to the wiki root. Default is to "
                               "generate the filename from the VCS. '-' will "
                               "write to stdout.")
...
def sync_static(config, args):
    ...
    command = ('rsync -vaxq --cvs-exclude --ignore-errors --include=.htaccess '
               '--exclude=.* --exclude=_*').split()
    display_cmd = command[:]
...
def sync_html(config, args):
    ...
    command = ('rsync -vaxq --cvs-exclude --delete --ignore-errors '
               '--include=.htaccess --exclude=.* --exclude=_*').split()
    display_cmd = command[:]
...
def build(config, args):
    ...
    for rel_filename in builder.walk():
        ...
        log.debug('Creating %s' % p.relpath(out_filename,
                                            start=config.temp_dir))
...
def build_listing(config, args):
    ...
    for fs_dir, _, _ in os.walk(config.html_dir):
        ...
        directory = '/' + '/'.join(p.relpath(fs_dir, start=config.html_dir)
                                   .split(p.sep))
        ...
        if not index_file_exists:
            log.debug("cp %s/%s %s/%s" % (directory, list_basename,
...
def serve(config, args):
    ...
    if args.interface:
        if not IPV4_RE.match(args.interface):
            serve.parser.error('invalid interface specifier: %r' %
                               args.interface)
...
serve.parser.add_argument('-i', '--interface', default=None,
                          help="Bind to specified interface (defaults to "
                          "loopback only)")
serve.parser.add_argument('-t', '--num-threads', type=int,
                          default=10, metavar='N',
                          help="Use N threads to handle requests (default is "
                          "10)")
serve.parser.add_argument('-n', '--server-name', default=None, metavar='NAME',
                          help="Use an explicit server name (default to an "
                          "autodetected value)")
serve.parser.add_argument('-q', '--queue-size', type=int, default=5,
                          metavar='SIZE',
                          help="Set request queue size (default is 5)")
serve.parser.add_argument('--timeout', type=int, default=10,
                          help="Set the socket timeout for connections "
                          "(default is 10)")
