...
class MarkdocWSGIApplication(object):
    ...
    def __call__(self, environ, start_response):
        ...
        self.log.info('%s %s - %d' % (request.method, request.path_info,
                                      response.status_int))
    ...
    def is_safe(self, directory):
        ...
        return p.pardir not in \
            p.relpath(directory, start=self.config.html_dir).split(p.sep)
    ...
    def directory(self, request):
        ...
        * Return a HTTP 404 ‘Not Found’.
        ...
        index_filename = p.join(self.config.html_dir, *(path_parts +
                                                        ['index.html']))
        ...
        if p.isfile(directory_filename) or p.isfile(directory_filename +
                                                    p.extsep + 'html'):
...
def serve_file(filename, content_type=None, chunk_size=4096):
    ...
    #     content_type = content_type.replace('text/html',
